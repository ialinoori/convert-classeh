...
const Page = () => {
  ...
  useEffect(() => {
    var url = `/api/v3/Assignmentresult?$filter={"andX":[{"eq":{"user.id":${
  ...
  const handleDeleteAssignment = () => {
    ...
    axios
      .post('https://mohammadfarhadi.classeh.ir//schoolservice/deleteassignment', deleteData)
      .then((response) => {
      ...
      <div className="bg-white mb-4 mx-auto w-[60%] shadow-md rounded-lg">
        ...
        {assignmentResult.length > 0 ? (
          <div className="mt-12">
            ...
            <h1 className="font-bold text-center  mb-8">اطلاعات پاسخ دانش اموز</h1>
 ...
 className="bg-red-600 rounded-md p-4 w-full text-white">پاک کردن پاسخ تکلیف</button>
          </div>
        ) : (
          <button
